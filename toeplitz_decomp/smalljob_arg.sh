...
n=$3
m=$4
p=$5            # VISAL SAYS: Can set to m/4, m/2, m, 2m. Fastest when set to m/2 or m/4.
pad=$6           # 0 for no padding; 1 for padding.

NP=$7          # Number of MPI processes. Must be set to 2n for this code. NP <= (RPN * bg_size)
RPN=$8          # Number of MPI processes per node = 1,2,4,8,16,32,64. RPN <= NP
OMP=$9           # Number of OpenMP threads per MPI process = 1,2,4,8,16,32,64. (RPN * OMP_NUM_THREADS ) <= 64 = threads per node
